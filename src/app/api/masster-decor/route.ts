...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #FCD34D; border-bottom: 2px solid #FCD34D; padding-bottom: 10px;">
          Nova Cotação - Linha Masster Decor
        </h2>
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; color: #6b7280; font-size: 12px;">
          ...
          <p>Este email foi enviado através do formulário de cotação da linha Masster Decor do site Masster Plásticos.</p>
        </div>
      </div>
    `;

    // Enviar email para a empresa
    const emailDestino = process.env.MASSTER_DECOR_EMAIL || process.env.CONTATO_EMAIL || 'gerentecomercial@massterplasticos.com.br';
    
    if (isEmailConfigured()) {
      const emailResult = await sendEmail({
        to: emailDestino,
        subject: 'Nova cotação - Linha Masster Decor',
        html: emailHtml,
      });
      ...
      console.log('Email não configurado. Nova cotação Masster Decor:', {
        nome,
        email,
    ...
    console.error('Erro ao processar cotação Masster Decor:', error);
    return NextResponse.json(
